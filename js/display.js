    ...
    if(state.status==="running"){
        ...
        if(questionChanged){
            ...
            if(state.noTimer===true){
                ...
                animateQuestionChange(targetText,state.timerSeconds||30);
            }
        }else{
            if(state.noTimer===true){
                clearTimer();
...
function animateQuestionChange(newQuestion,seconds){
...
display.classList.add("question-exit");

setTimeout(()=>{
    if(!display)return;

    display.textContent=newQuestion;

    display.classList.remove("question-exit");
    ...
    requestAnimationFrame(()=>{
        requestAnimationFrame(()=>{
            ...
            display.classList.add("question-enter-active");
...
style.textContent=`
    ...
    #questionDisplay.question-exit{
        animation:questionSlideOut .4s cubic-bezier(.7,0,.9,.5) forwards;
    }

    #questionDisplay.question-enter{
        opacity:0;
        transform:translateX(90px);
        filter:blur(14px);
    }

    #questionDisplay.question-enter-active{
        animation:questionSlideIn .55s cubic-bezier(.16,1,.3,1) forwards;
    }
...
for(let i=0;i<90;i++){
    ...
    confetti.style.height=`${size*.6}px`;
