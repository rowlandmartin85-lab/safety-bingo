...
function initializeHostMain() {
  if (hostMainInitialized) {
    console.warn("HOST MAIN ALREADY INITIALIZED");
    return;
  }
  ...
  console.log("INITIALIZING HOST MODULES...");
  console.log("==========================================");
   ...
   * created there.
   */
  initializeHostModule("HOST STATE", window.initializeHostState);
   ...
   * -------------------------------------------------
   */
  initializeHostModule("HOST UI", window.initializeHostUI);
   ...
   */
  initializeHostModule("HOST GAME", window.initializeHostGame);
   ...
   * -------------------------------------------------
   */
  initializeHostModule("HOST CHECKER", window.initializeHostChecker);
   ...
   * -------------------------------------------------
   */
  initializeHostModule("HOST AUDIT", window.initializeHostAudit);
   ...
   * -------------------------------------------------
   */
  initializeHostModule("HOST PRINTER", window.initializeHostPrinter);
...
function initializeHostModule(moduleName, initializer) {
  if (typeof initializer !== "function") {
     ...
     */
    if (moduleName !== "HOST STATE") {
      console.error(`${moduleName} MISSING`);
    } else {
      console.warn("HOST STATE INITIALIZER NOT FOUND");
    }
...
function verifyHostElements() {
  ...
  if (missing.length > 0) {
    console.warn("HOST HTML ELEMENTS MISSING:", missing);
  } else {
    console.log("HOST HTML ELEMENT CHECK: OK");
  }
}
...
function initializeHomeButton() {
   ...
   * -------------------------------------------------
   */
  if (homeModal && homeModal.dataset.outsideReady !== "true") {
    homeModal.dataset.outsideReady = "true";
  ...
  if (confirmHome && homeModal && confirmHome.dataset.homeReady !== "true") {
    ...
    confirmHome.addEventListener("click", () => {
      leaveHostGame(confirmHome, homeModal);
    });
  }
}
...
function leaveHostGame(confirmHome, homeModal) {
  console.log("==========================================");
  ...
  if (confirmHome) {
    ...
    confirmHome.textContent = "ENDING GAME...";
  }
  ...
  if (socket && typeof socket.emit === "function") {
    console.log("HOST SOCKET FOUND:", socket.id);
    console.log("SENDING hostLeftGame");

    try {
      socket.emit("hostLeftGame");
    } catch (error) {
      console.error("HOST LEFT GAME EMIT ERROR:", error);
    }
  } else {
    console.warn("HOST SOCKET NOT AVAILABLE");
  }
   ...
   */
  setTimeout(() => {
    const currentSocket = window.hostSocket;

    if (
      currentSocket &&
      typeof currentSocket.disconnect === "function"
    ) {
      ...
      try {
        ...
        console.warn("HOST SOCKET DISCONNECT ERROR:", error);
      }
    }
